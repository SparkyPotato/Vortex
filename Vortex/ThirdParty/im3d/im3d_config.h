...
#define IM3D_VEC2_APP \
	Vec2(const Vortex::Math::Vector& _v)          { x = _v.x; y = _v.y;     } \
	operator Vortex::Math::Vector() const         { return Vortex::Math::Vector(x, y, 0.f); }
#define IM3D_VEC3_APP \
	Vec3(const Vortex::Math::Vector& _v)          { x = _v.x; y = _v.y; z = _v.z; } \
	operator Vortex::Math::Vector() const         { return Vortex::Math::Vector(x, y, z);    }
#define IM3D_VEC4_APP \
	Vec4(const Vortex::Math::Vector& _v)          { x = _v.x; y = _v.y; z = _v.z; w = _v.w; } \
	operator Vortex::Math::Vector() const         { return Vortex::Math::Vector(x, y, z, w);           }
